...
public class FileController {
	...
	public String addok(Model model, String txt, MultipartFile attach, HttpServletRequest req) {
		...
		try {
			...
			//2. 파일명 중복 방지
			//	a. 숫자 붙이기
			//	b. 고유 접두어 붙이기 : 시간_파일명. 난수_파일명 
			//	c. UUID(Universally Unique ID)
			//		- 네트워크 상에서 고유성이 보장되는 ID를 만들기 위한표준
			//		- 시간 + 난수
			...
			//21743813459200_highlight.zip
			//21743813459345_highlight.zip
			
			UUID uuid = UUID.randomUUID();
			System.out.println(uuid);
			
			//1a9c71e9-54e8-4527-94cb-559c1dc734e6_파일명
	...
	public String view(Model model, String seq) {
		...
		if(pdto != null) {
			...
			try {
				...
				if(gps.containsTag(GpsDirectory.TAG_ALTITUDE) && gps.containsTag(GpsDirectory.TAG_LONGITUDE)) {
